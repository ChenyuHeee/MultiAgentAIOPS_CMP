...
from typing import Any, Dict, List, Optional, Sequence, Tuple # 类型声明
...
_SentenceModelType = Any

_sentence_model: Optional[_SentenceModelType] = None
...
def _load_sentence_model() -> _SentenceModelType:
    if SentenceTransformer is None:  # pragma: no cover - optional dependency
        raise RuntimeError(
            "sentence_transformers 未安装，无法进行语义相似度评估。请安装 sentence-transformers 包。"
        )
    ...
    with _sentence_model_lock:
        ...
        model_name = os.getenv("AIOPS_SENTENCE_MODEL", "paraphrase-multilingual-MiniLM-L12-v2")
        device = "cpu"
        if torch is not None:  # pragma: no branch - small utility
            try:
...
def _semantic_similarity(text_a: str, text_b: str) -> float:
    if not text_a or not text_b:
        return 0.0
    if cosine is None:  # pragma: no cover - optional dependency
        raise RuntimeError(
            "scipy 未安装，无法计算余弦距离。请安装 scipy 包。"
        )
    ...
    except Exception:  # pragma: no cover - 推理失败
        return 0.0

    try:
        distance = cosine(emb_a, emb_b)
    except Exception:  # pragma: no cover - 数值问题
        return 0.0
...
def reason_matches(gt_reason: str, submission_reason: str, keywords: Sequence[str], threshold: float) -> bool: # 判断提交理由是否与Ground Truth一致，gt_reason为Ground Truth的理由，submission_reason为提交理由，keywords为关键词列表，threshold为相似度阈值
    ...
    try:
        similarity = _semantic_similarity(trimmed_submission, trimmed_gt)
    except RuntimeError as exc: # 依赖缺失或模型加载失败时回退为不通过
        if not _semantic_warning_emitted:
            print(
                f"[WARN] 无法加载语义相似度模型（{exc}），reason 匹配将视为不通过。",
                file=sys.stderr,
            )
...
def score_submission( # 计算提交分数
    ...
    for uuid, gt_entry in ground_truth.items(): # 遍历每个 uuid 和对应的 Ground Truth 记录
        ...
        # 组件判定：选手提交的 component 需要与 GT 中的任一实例完全相等（不做子串匹配）
        gt_component_parts = [part.strip() for part in gt_component.replace("->", "+").split("+") if part.strip()]
        component_correct = submission_component in gt_component_parts if gt_component_parts else False
...
# 打印结果
    ...
    if args.report: # 如果有报告，则保存
        def _json_default(o: object):
            # numpy scalars (e.g., bool_) are not JSON serializable by default
            try:
        ...
        args.report.write_text(json.dumps(report_payload, indent=2, default=_json_default), encoding="utf-8")
