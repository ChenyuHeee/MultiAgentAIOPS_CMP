...
_ALERT_ENDPOINT_RE = re.compile(r"Alert:\s*Endpoint\s+(.+?)\s+experiencing", re.IGNORECASE)


def _fallback_final_answer(history: str) -> str:
    m = _ALERT_ENDPOINT_RE.search(history or "")
    ep = m.group(1).strip() if m else ""
    if ep:
        return (
            f"Root Cause Endpoint: {ep}\n"
            "Root Cause Reason: Max steps reached; fallback to alert endpoint."
        )
    return "Root Cause Endpoint: \nRoot Cause Reason: Max steps reached."
...
class ReActTotRun(BaseRun):
    ...
    def run(self, agent: AgentWorkflow, question: str, agent_tool_env, eval_run, agents, history="", index=0):
        ...
        # Safety cap to avoid infinite ReAct loops on some models/endpoints.
        if depth >= REACT_MAX_STEPS:
            return _fallback_final_answer(history)

        step_status_record_list = self.sample_multi_next_step(agent, question, agent_tool_env, eval_run, agents, history)
        # NOTE: TOT_CHILDREN_NUM defaults to 1; selection index should not be tied to recursion depth.
        ...
        if best_step_status_record["status"] != REACT_STATUS_FINISH:
            return self.run(agent, question, agent_tool_env, eval_run, agents, history, depth + 1)
        else:
