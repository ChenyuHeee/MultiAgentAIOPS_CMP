...
# minute: String type. Accepted formats: "YYYY-MM-DD HH:MM" or "YYYY-MM-DD HH:MM:SS".
# The tool will normalize to second precision internally.
...
def query_endpoint_stats(endpoint: str, minute: str) -> dict:
    ...
    - minute (str): The specific time at which statistics are to be queried.
...
def query_endpoint_metrics_in_range(endpoint: str, minute: str) -> dict:
    ...
    Parameters:
    - endpoint (str): The unique identifier of the API endpoint to be queried. 
    - minute (str): The central time point around which the statistics are to be queried.
      Accepted formats: "YYYY-MM-DD HH:MM" or "YYYY-MM-DD HH:MM:SS".

    Notes:
    - The tool tolerates endpoint naming variants (e.g., punctuation differences, optional "service" token)
      and may aggregate metrics across multiple matched endpoints.
